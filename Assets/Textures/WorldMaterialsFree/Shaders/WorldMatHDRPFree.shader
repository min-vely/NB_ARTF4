	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 texCoord21 = inputMesh.ase_texcoord.xy * temp_cast_0 + float2( 0,0 );
				...
				#endif
				float3 vertexValue = ( ( tex2Dlod( _Height, float4( texCoord21, 0, 1.0) ) * float4( inputMesh.normalOS , 0.0 ) ) * _HeightDisplacement ).rgb;
						...
						)
			{
				...
				float2 temp_cast_0 = (_Tile).xx;
				float2 texCoord21 = packedInput.ase_texcoord5.xy * temp_cast_0 + float2( 0,0 );
				
				float3 unpack7 = UnpackNormalScale( tex2D( _Normal, texCoord21 ), _NormalScale );
				unpack7.z = lerp( 1, unpack7.z, saturate(_NormalScale) );
				
				surfaceDescription.Albedo = ( tex2D( _Albedo, texCoord21 ) * _ColorTint ).rgb;
				surfaceDescription.Normal = unpack7;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( tex2D( _Metallic, texCoord21 ) * _MetallicStrength ).r;
				...
				surfaceDescription.Smoothness = ( tex2D( _Smoothness, texCoord21 ) * _SmoothnessStrength ).r;
				surfaceDescription.Occlusion = tex2D( _AO, texCoord21 ).r;
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float2 temp_cast_0 = (_Tile).xx;
				float2 texCoord21 = inputMesh.uv0.xy * temp_cast_0 + float2( 0,0 );
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 temp_cast_0 = (_Tile).xx;
				float2 texCoord21 = packedInput.ase_texcoord.xy * temp_cast_0 + float2( 0,0 );
		...
		Pass
		{
					...
					)
			{
				...
				float2 temp_cast_0 = (_Tile).xx;
				float2 texCoord21 = packedInput.ase_texcoord3.xy * temp_cast_0 + float2( 0,0 );
				float3 unpack7 = UnpackNormalScale( tex2D( _Normal, texCoord21 ), _NormalScale );
				unpack7.z = lerp( 1, unpack7.z, saturate(_NormalScale) );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 temp_cast_0 = (_Tile).xx;
				float2 texCoord21 = packedInput.ase_texcoord7.xy * temp_cast_0 + float2( 0,0 );
...
WireConnection;24;4;19;0
WireConnection;24;7;23;0
WireConnection;24;8;5;0
WireConnection;24;11;13;0
ASEEND*/
//CHKSM=314D0A175474C14D2BF92CD97A9CD4735306783F